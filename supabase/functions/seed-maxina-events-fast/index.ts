...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Seeding 40 events (fast) for user ${user.id}`);
    ...
    console.log(`✓ Seeded ${inserted.length} events (fast)`);

    return new Response(
      JSON.stringify({
        success: true,
        message: `Seeded ${inserted.length} Maxina Summer 2026 events (fast)` ,
        events: inserted.map((e: any) => ({ id: e.id, title: e.title }))
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 }
    );
  } catch (error) {
    console.error('Seed fast error:', error);
    return new Response(
