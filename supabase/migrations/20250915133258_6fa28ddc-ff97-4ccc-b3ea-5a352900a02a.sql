...
-- Drop any existing vulnerable policies
DROP POLICY IF EXISTS "Medical data access restricted" ON public.profiles;
DROP POLICY IF EXISTS "Secure profile access with medical data protection" ON public.profiles;
...
CREATE POLICY "Profiles secure medical data access v2" 
ON public.profiles 
FOR SELECT 
USING (
  auth.uid() = user_id OR 
