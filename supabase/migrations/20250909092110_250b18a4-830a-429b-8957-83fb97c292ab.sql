-- First, drop ALL existing policies on memberships table
DROP POLICY IF EXISTS "memberships_select_safe" ON public.memberships;
DROP POLICY IF EXISTS "memberships_insert_safe" ON public.memberships;  
DROP POLICY IF EXISTS "memberships_update_safe" ON public.memberships;
DROP POLICY IF EXISTS "memberships_delete_safe" ON public.memberships;
...
CREATE POLICY "memberships_select_new" 
...
CREATE POLICY "memberships_insert_new" 
ON public.memberships 
FOR INSERT 
WITH CHECK (
...
CREATE POLICY "memberships_update_new" 
ON public.memberships 
FOR UPDATE 
USING (
  public.is_exafy_admin(auth.uid())
...
CREATE POLICY "memberships_delete_new" 
ON public.memberships 
FOR DELETE 
USING (
  public.is_exafy_admin(auth.uid())
