...
-- Add indexes for better performance on last message queries (without CONCURRENTLY)
CREATE INDEX IF NOT EXISTS idx_global_messages_thread_created_at 
ON public.global_messages(thread_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_messages_thread_created_at 
ON public.messages(thread_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_global_threads_updated_at 
