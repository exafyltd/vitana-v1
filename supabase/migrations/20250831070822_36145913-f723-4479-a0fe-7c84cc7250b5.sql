-- Fix the bootstrap_admin_user function to use the correct column name
...
BEGIN
  ...
  UPDATE auth.users 
  SET raw_app_meta_data = COALESCE(raw_app_meta_data, '{}'::jsonb) || jsonb_build_object('role', 'exafy_admin')
  WHERE id = user_id;
