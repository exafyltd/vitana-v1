...
-- Global messages: Allow thread participants to update delivered_at and read_at
...
FOR UPDATE USING (
  ...
  -- Only allow updating delivered_at and read_at columns
  delivered_at IS NOT DISTINCT FROM OLD.delivered_at OR
  read_at IS NOT DISTINCT FROM OLD.read_at
);

-- Tenant messages: Allow thread participants to update delivered_at and read_at
