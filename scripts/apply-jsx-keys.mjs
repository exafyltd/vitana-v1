...
// Prefer the most recent manifest (Wave 6 AST → Wave 5 → Wave 4 → Wave 3).
const MANIFEST_W6 = join(__dirname, '.codemod-jsx-ast.manifest.json');
const MANIFEST_W5 = join(__dirname, '.codemod-jsx-expr.manifest.json');
const MANIFEST_W4 = join(__dirname, '.codemod-jsx-v2.manifest.json');
const MANIFEST_W3 = join(__dirname, '.codemod-jsx.manifest.json');
const MANIFEST = existsSync(MANIFEST_W6) ? MANIFEST_W6
              : existsSync(MANIFEST_W5) ? MANIFEST_W5
              : existsSync(MANIFEST_W4) ? MANIFEST_W4
              : MANIFEST_W3;
