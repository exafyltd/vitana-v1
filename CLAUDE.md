...
**Moved to `.claude/rules/infrastructure.md`** (2026-09-22) — loaded
automatically when a session touches `.github/workflows/**`. Covers AWS
production hosts, the staging-first cutover, GCP-billing-off staging
...
**Moved to `.claude/rules/frontend.md`** (2026-09-22) — loaded
automatically when a session touches `.env`/`.env.example`/the TTS files
it discusses (`gateway-tts.ts`, `useTextToSpeech.ts`,
`VoiceSettingsPanel.tsx`). Covers the `VITE_*` build-time env vars and the
frontend-TTS-through-the-gateway (Polly) resolution note. Pure
relocation, nothing rewritten or summarized.
