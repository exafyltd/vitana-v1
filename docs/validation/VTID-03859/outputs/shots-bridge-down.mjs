...
async function shot(name, { role, lang, path, rtl, click, steps }) {
  ...
  for (const st of steps || []) {
    try {
      if (st.click) { await page.locator(st.click).first().click({ timeout: 10000 }); await page.waitForTimeout(st.wait ?? 500); }
...
await shot('06-draft-lead-bridge-down-salesmanager-en', { ...M, path: '/backoffice/sales/leads', steps: [
  { click: '[data-testid="draft-open-lead"]', wait: 800 }, { fill: 'input[name="lead_name"]', value: 'Bridge Down Probe' }, { click: '[data-testid="draft-review"]', wait: 600 }, { click: '[data-testid="draft-accept"]' }, { waitFor: '[data-testid="draft-failed"]', wait: 800 }, { shot: '-1-failed' }] });
await shot('07-draft-buttons-hidden-salesperson-en', { role: 'backoffice', lang: 'en-US', path: '/backoffice/sales/leads' });
