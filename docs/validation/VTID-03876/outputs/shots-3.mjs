...
const ONLY = ["05-edit-opportunity-en", "06-edit-task-de", "07-edit-lead-ar-rtl"];
...
const M = { role: 'admin', lang: 'en-US' };   // stub persona decides the capabilities; `role` only drives the app's own view switch
const LEADS = '/backoffice/sales/leads', TASKS = '/backoffice/sales/followups', OPPS = '/backoffice/sales/opportunities';

await shot('01-edit-lead-en', { ...M, path: LEADS, steps: [
  { click: 'table tbody tr', wait: 1200 }, { shot: '-1-detail-with-edit' },
  { click: '[data-testid="draft-open-leadUpdate"]', wait: 1200 }, { shot: '-2-form-prefilled' },
  { select: 'status', option: 'contacted' }, { fill: 'input[name="phone"]', value: '+971 50 000 0001' },
  { click: '[data-testid="draft-review"]', wait: 600 }, { shot: '-3-review' },
  { click: '[data-testid="draft-accept"]' }, { waitFor: '[data-testid="draft-done"]', wait: 800 }, { shot: '-4-saved' } ] });

await shot('02-complete-task-en', { ...M, path: TASKS, steps: [
  { shot: '-1-actions-per-open-task' },
  { click: '[data-testid="draft-open-taskComplete"]', wait: 1200 }, { shot: '-2-form' },
  { click: '[data-testid="draft-review"]', wait: 600 }, { shot: '-3-review' },
  { click: '[data-testid="draft-accept"]' }, { waitFor: '[data-testid="draft-done"]', wait: 800 }, { shot: '-4-done' } ] });

await shot('03-cancel-task-en', { ...M, path: TASKS, steps: [
  { click: '[data-testid="draft-open-taskCancel"]', wait: 1200 },
  { click: '[data-testid="draft-review"]', wait: 600 }, { shot: '-1-reason-required' },
  { fill: 'textarea[name="reason"]', value: 'Superseded by the new proposal' },
  { click: '[data-testid="draft-review"]', wait: 600 }, { shot: '-2-review' },
  { click: '[data-testid="draft-accept"]' }, { waitFor: '[data-testid="draft-done"]', wait: 800 }, { shot: '-3-cancelled' } ] });

await shot('04-move-stage-en', { ...M, path: OPPS, steps: [
  { shot: '-1-board-card-actions' },
  { click: '[data-testid="draft-open-opportunityStage"]', wait: 1200 }, { shot: '-2-form' },
  { select: 'stage', option: 'Negotiation' }, { click: '[data-testid="draft-review"]', wait: 600 }, { shot: '-3-review' },
  { click: '[data-testid="draft-accept"]' }, { waitFor: '[data-testid="draft-done"]', wait: 800 }, { shot: '-4-moved' } ] });

await shot('05-edit-opportunity-en', { ...M, path: OPPS, steps: [
  { click: '[data-testid="draft-open-opportunityUpdate"]', wait: 1200 }, { shot: '-1-form-no-stage' },
  { fill: 'input[name="probability"]', value: '75' }, { click: '[data-testid="draft-review"]', wait: 600 }, { shot: '-2-review' } ] });

await shot('06-edit-task-de', { role: 'admin', lang: 'de-DE', path: TASKS, steps: [
  { click: '[data-testid="draft-open-taskUpdate"]', wait: 1200 }, { shot: '-1-form' },
  { click: '[data-testid="draft-review"]', wait: 600 }, { shot: '-2-review' } ] });

await shot('07-edit-lead-ar-rtl', { role: 'admin', lang: 'ar-XA', rtl: true, path: LEADS, steps: [
  { click: 'table tbody tr', wait: 1200, dispatch: true },
  { click: '[data-testid="draft-open-leadUpdate"]', wait: 1200, dispatch: true }, { shot: '-1-form' } ] });

await shot('08-no-actions-salesperson-en', { ...M, path: TASKS, steps: [ { shot: '-1-tasks' } ] });
await shot('09-no-edit-salesperson-en', { ...M, path: LEADS, steps: [ { click: 'table tbody tr', wait: 1200 }, { shot: '-1-lead-detail' } ] });
