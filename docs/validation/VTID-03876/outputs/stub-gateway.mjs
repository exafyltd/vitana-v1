...
const readShape = (n) => {
  const f = fs.existsSync(`${SH}/${n}.txt`) ? `${SH}/${n}.txt` : `${SH2}/${n}.txt`;
...
const caps = PERSONA === 'sales' ? ['crm.view','sales.view'] : PERSONA === 'salesmanager' ? ['crm.view','crm.manage','sales.view','sales.draft'] : PERSONA === 'bookkeeper'
...
http.createServer((req, res) => {
  ...
  if (u.pathname === '/api/v1/backoffice/me') { body = me; status = 200; }
    ...
    else if (ap0) { body = { ok: true, command: { command_id: id, type: 'sales.credit_note.submit', action: 'submit-credit-note', tier: 'high', status: ap0.status === 'pending' ? 'awaiting_approval' : ap0.status === 'approved' ? 'executed' : 'rejected', reason: ap0.status === 'pending' ? 'awaiting_approval' : null, approval_id: ap0.id, receipt: null, escalations: ['kind:credit_note'], channel: 'web', requester_id: ap0.requester_id, created_at: ap0.created_at, executed_at: null, replayed: false } }; status = 200; }
    ...
    let raw = ''; req.on('data', d => raw += d); req.on('end', () => {
      ...
      const type = b.type; const isDraft = DRAFT_TYPES.has(type); const base = { command_id: `stub-${posts}`, type, action: RESULTS[type] || type, tier: isDraft ? 'draft' : 'read', channel: 'web', requester_id: ME_ID, created_at: new Date().toISOString(), escalations: [], approval_id: null, replayed: false };
      if (isDraft) console.log('DRAFT', type, 'key=' + b.idempotency_key, 'confirm=' + b.confirm, 'payload=' + JSON.stringify(b.payload));
