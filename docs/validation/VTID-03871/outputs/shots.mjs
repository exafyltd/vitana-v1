...
await shot('01-draft-payment-admin-en', { ...A, path: '/backoffice/finance/payments', steps: [
  { click: '[data-testid="draft-open-payment"]', wait: 1500 }, { shot: '-1-form-empty' },
  { click: '[data-testid="draft-review"]', wait: 400 }, { shot: '-2-validation' },
  { select: 'party_id', option: 'Acme Trading' }, { fill: 'input[name="paid_amount"]', value: '1050' }, { select: 'paid_from_account', option: '1120' }, { select: 'paid_to_account', option: '1110' }, { fill: 'input[name="reference_number"]', value: 'TRF-2026-0913-01' }, { fill: 'input[name="reference_date"]', value: '2026-09-13' }, { shot: '-3-form-filled' },
  { click: '[data-testid="draft-review"]', wait: 800 }, { shot: '-4-review-card' }, { click: '[data-testid="draft-accept"]' }, { waitFor: '[data-testid="draft-done"]', wait: 800 }, { shot: '-5-done' }] });
await shot('02-draft-payment-admin-de', { role: 'admin', lang: 'de-DE', path: '/backoffice/finance/payments', steps: [
  { click: '[data-testid="draft-open-payment"]', wait: 1500 }, { select: 'party_id', option: 'Acme Trading' }, { fill: 'input[name="paid_amount"]', value: '1050' }, { select: 'paid_from_account', option: '1120' }, { select: 'paid_to_account', option: '1110' }, { click: '[data-testid="draft-review"]', wait: 800 }, { shot: '-1-review-card' }] });
await shot('03-draft-payment-admin-ar-rtl', { role: 'admin', lang: 'ar-XA', rtl: true, path: '/backoffice/finance/payments', steps: [
  { click: '[data-testid="draft-open-payment"]', wait: 1500, dispatch: true }, { select: 'paid_from_account', option: '1120' }, { shot: '-1-form' }] });
