...
async function measure(name, lang, rtl) {
  const ctx = await browser.newContext({ viewport: { width: 1400, height: 900 }, locale: rtl ? 'ar-AE' : lang, serviceWorkers: 'block' });
  const page = await ctx.newPage();
  await page.route('**/*', async (route) => {
    const req = route.request(); const url = req.url(); const m = req.method();
    if (url.includes('/auth/v1/user') && m === 'GET') return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(session.user) });
    if (url.includes('/auth/v1/token')) return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(session) });
    if (url.includes('/rest/v1/rpc/get_role_preference')) return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify([{ role: 'admin' }]) });
    if (url.includes('/rest/v1/rpc/get_my_permitted_roles')) return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ ok: true, roles: ['community', 'admin'], is_super_admin: false }) });
    if (url.startsWith('http://127.0.0.1:8090/')) { if (m !== 'GET') { const ok = m === 'POST' && url.endsWith('/api/v1/backoffice/commands'); if (!ok) return route.abort(); } return route.continue(); }
    if (url.includes('supabase.co') || url.includes('vitanaland.com') || url.includes('/api/v1/')) {
      const readRpc = /\/rest\/v1\/rpc\/(get_|list_|current_|check_|search_|fetch_|is_|has_)/.test(url);
      if (m !== 'GET' && m !== 'HEAD' && m !== 'OPTIONS' && !readRpc && !url.includes('/auth/v1/')) return route.abort();
    }
    return route.continue();
  });
  await page.goto(`${APP}/auth`, { waitUntil: 'domcontentloaded' });
  await page.evaluate(({ s, lang }) => { localStorage.setItem('sb-inmkhvwdcuyhnxkgfvsb-auth-token', JSON.stringify(s)); localStorage.setItem('vitana.authToken', s.access_token); localStorage.setItem('vitana.viewRole', 'community'); localStorage.setItem('vitana.lang', lang); localStorage.setItem('vitana::global::dev::language::selected_language', lang); }, { s: session, lang });
  await page.goto(`${APP}/backoffice/sales/followups`, { waitUntil: 'domcontentloaded' });
  await page.locator('[data-screen-id]').first().waitFor({ timeout: 60000 }).catch(() => {});
  ...
  const geo = await page.evaluate(() => {
    ...
    const inset = document.querySelector('main')?.closest('[class*="flex"]');
    const box = (el) => el ? (({x,width}) => ({ x: Math.round(x), right: Math.round(x+width), w: Math.round(width) }))(el.getBoundingClientRect()) : null;
    const h1 = document.querySelector('h1');
    const table = document.querySelector('table');
    // what element actually receives a click at the left edge of the content area
    const probeX = dir === 'rtl' ? Math.round(window.innerWidth - 20) : 20;
    const atContentEdge = (() => { const b = (h1||table)?.getBoundingClientRect(); if (!b) return null;
      const el = document.elementFromPoint(Math.round(b.x + 5), Math.round(b.y + 5)); return el ? (el.closest('[data-sidebar]') ? 'SIDEBAR' : el.tagName + '.' + String(el.className).slice(0,40)) : null; })();
    return { dir, panel: box(panel), spacer: box(spacer), main: box(document.querySelector('main')), h1: box(h1), table: box(table), atContentEdge, vw: window.innerWidth };
  });
  console.log(JSON.stringify({ name, ...geo }));
  await page.screenshot({ path: `${OUT}/${name}.png` });
  await ctx.close();
}
await measure('geo-ltr', 'en-US', false);
await measure('geo-rtl', 'ar-XA', true);
await browser.close();
