// VTID-03849 screenshot stub: the not-yet-deployed gateway BackOffice routes, mocked to the router's exact shapes.
...
http.createServer((req, res) => {
  ...
  if (u.pathname === '/api/v1/backoffice/me') { body = me; status = 200; }
    ...
    let raw = ''; req.on('data', d => raw += d); req.on('end', () => {
      ...
      else { out = { ok: true, command: { ...base, status: 'executed', reason: null, executed_at: new Date().toISOString(), receipt: receipt(RESULTS[type], readShape(RESULTS[type]), 300 + posts) } }; st = 200; }
