...
Generated: 2026-08-31T09:57:18.906Z via **gemini**
...
- `billing.json:billing.features.priorityPractitioner` — **LOW_CONFIDENCE** (0.60) — The term '访问' (access) is ambiguous; it could mean 'access to practitioners' or 'access for practitioners'. '从业者' (practitioner) is also a bit formal. More context is needed for a precise translation.
- `billing.json:paywall.live_room_minutes.title` — **EDIT_SUGGESTED** (0.90) — '不足' (insufficient) is too formal for a friendly app. 'Running low' implies nearing depletion.
  - suggested: `主持时间快用完了`
- `billing.json:paywall.live_room_minutes.body` — **EDIT_SUGGESTED** (0.90) — '该房间' (this room) and '以' (in order to) are too formal. 'Keep it running' is better conveyed with a more direct phrase.
  - suggested: `这个房间还剩 5 分钟主持时间。添加主持时间或升级，让它继续运行。`
- `billing.json:paywall.match_reveals.title` — **EDIT_SUGGESTED** (0.80) — '揭示' (reveal) can sound overly formal or dramatic for a simple 'reveal match' action in an app. '查看' (view) is more common and natural.
  - suggested: `查看匹配`
