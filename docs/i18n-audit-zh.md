...
Generated: 2026-09-12T08:01:38.862Z via **gemini**
...
- `billing.json:billing.features.priorityPractitioner` — **EDIT_SUGGESTED** (0.90) — The term '访问' (access) is ambiguous here. It can mean 'visit' or 'access to a system/website', but for 'access to a practitioner', it's less idiomatic and could imply the practitioner is doing the accessing. '联系' (contact) or '获得服务' (get service) is more appropriate.
  - suggested: `优先联系从业者`
- `billing.json:paywall.match_reveals.title` — **EDIT_SUGGESTED** (0.90) — The term '揭示' (reveal) is a bit formal and dramatic for a typical UI action in a friendly app. Given that 'reveals' (noun) in entry 7 is translated as '查看次数' (view/check times), '查看匹配' (view match) would be more consistent and natural for a UI button.
  - suggested: `查看匹配`
...
- `mobileSupport.json:mobilesupport.description` — **LOW_CONFIDENCE** (0.00) — audit failed: Gemini HTTP 429: {
  "error": {
    "code": 429,
    "message": "You exceeded your current quota, please check your plan and billing details. For more information on this error, head to: https://ai.google.dev/gemini-api/docs/rate-limits. To monitor your current usage, head to: https://ai.dev/rate-limit. \n* Quota ex
- `newsCard.json:newsCard.detail.continueReading` — **EDIT_SUGGESTED** (0.90) — The phrase '在以下位置' for 'on' is too literal and formal for a friendly app's register; it's not idiomatic.
  - suggested: `继续阅读`
