/**
 * VTID-02601 / BOOTSTRAP-REMINDER-DEEPLINK — Reusable reminders panel.
 *
 * Extracted from the standalone /reminders page so the same UI can be embedded
 * as a tab inside the Calendar surface. Pass `embedded` to drop the outer page
 * container + big heading (the calendar tab supplies its own chrome).
 *
 * Layered model:
 *   1. Suggested by Vitana — created_via='system'
  ...
  return (
    <div className="flex items-center justify-between gap-3 p-3 rounded-lg border bg-card">
      ...
      <div className="flex items-center gap-2 shrink-0">
        {reminder.status === "pending" || reminder.status === "dispatching" ? (
          <Button variant="ghost" size="icon" onClick={() => onComplete(reminder)} title={t('screens.reminders.markDone')}>
        ...
        <Button
          ...
          title={t('screens.reminders.delete')}
        >
...
export interface RemindersPanelProps {
  /** When true, drop the outer page container/spacing — for embedding in a
   *  calendar tab or other host that supplies its own chrome. */
  embedded?: boolean;
}

const RemindersPanel: React.FC<RemindersPanelProps> = ({ embedded = false }) => {
  ...
  const filteredView: { title: string; items: ReminderRow[]; emptyText: string } | null = useMemo(() => {
    switch (activeFilter) {
      case "upcoming":
        return { title: t('screens.reminders.filterUpcoming'), items: upcoming, emptyText: t('screens.reminders.noUpcoming') };
      case "completed":
        return { title: t('screens.reminders.filterCompleted'), items: completed, emptyText: t('screens.reminders.noCompleted') };
      case "missed":
        return { title: t('screens.reminders.filterMissed'), items: missed, emptyText: t('screens.reminders.noMissed') };
      default:
        return null;
    }
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      notifyError('toasts.reminders.failedToCreate');
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      notifyError('toasts.reminders.failedToDelete');
    } finally {
  ...
  const handleDeleteAll = async () => {
    try {
      ...
      notifySuccess('toasts.reminders.remindersDeleted', { count: r.deleted });
    } catch {
      notifyError('toasts.reminders.failedToDelete');
    } finally {
  ...
  const handleComplete = async (r: ReminderRow) => {
    try {
      ...
      notifyError('toasts.reminders.failedToMarkDone');
    }
  };
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreate} disabled={createMut.isPending}>
              {createMut.isPending ? t('screens.reminders.creating') : t('screens.reminders.createReminder')}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
