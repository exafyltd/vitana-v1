  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-col items-center px-6 py-10 gap-6 max-w-md w-full mx-auto">
        ...
        <div className="text-lg font-extrabold tracking-wide text-[#1B8FC7] uppercase text-center">{game.name}</div>

        <EventGameCountdown endsAt={game.ends_at} className="text-center" />

        <div className="grid grid-cols-2 gap-4 w-full text-center">
          <div className="relative overflow-hidden rounded-2xl bg-white shadow-[0_8px_24px_rgba(31,143,199,0.25)] p-5">
            <div className="text-xs uppercase tracking-wide text-muted-foreground">{t('eventGame.home.score')}</div>
            <div className="text-5xl font-extrabold bg-gradient-to-r from-[#6CC5EC] to-[#1B8FC7] bg-clip-text text-transparent">{score}</div>
          ...
          <div className="relative overflow-hidden rounded-2xl bg-white shadow-[0_8px_24px_rgba(31,143,199,0.25)] p-5">
            ...
            <div className="text-5xl font-extrabold bg-gradient-to-r from-[#6CC5EC] to-[#1B8FC7] bg-clip-text text-transparent">
              {rank === 1 ? '#1' : `#${rank || '—'}`}
            </div>
            <DecorativeWave className="pointer-events-none absolute bottom-0 left-0 w-full h-6 text-[#E3F5FD]" />
          </div>
        ...
        <Button
          size="lg"
          className="w-full text-lg h-14 rounded-full bg-gradient-to-r from-[#6CC5EC] to-[#1B8FC7] hover:from-[#8FD5FA] hover:to-[#3AA6D6] shadow-[0_10px_28px_rgba(31,143,199,0.45)] hover:shadow-[0_12px_32px_rgba(31,143,199,0.55)] transition-all duration-300 border-0 flex items-center justify-center gap-2"
          onClick={() => setPostOpen(true)}
        >
          {t('eventGame.home.postCta')}
          <ArrowRight className="w-5 h-5" />
        </Button>
        <div className="text-xs text-[#4A7688] -mt-4">
        ...
        <div className="flex gap-3 w-full">
          <Button
            variant="outline"
            className="flex-1 rounded-full bg-white/70 backdrop-blur border-white text-[#1B8FC7] hover:bg-white hover:text-[#1B8FC7] flex items-center justify-center gap-1.5"
