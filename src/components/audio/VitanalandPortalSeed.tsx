...
interface VitanalandPortalSeedProps {
  ...
  size?: 'sm' | 'nav' | 'md' | 'lg';
  layoutId?: string;
  glowIntensity?: number; // 0-1 range, default 1. 0 = no external halos (mobile readability mode)
}
...
export function VitanalandPortalSeed({ 
  ...
  return (
    <motion.div 
      ...
      layoutId={layoutId}
    >
      {/* External halo layers - conditionally rendered based on glowIntensity */}
      {glowIntensity > 0 && (
        <>
          ...
          <motion.div
            ...
            animate={{
              scale: haloScale,
              opacity: (isListening ? [0.8, 1, 0.8] : [0.9, 1, 0.9]).map(v => v * glowIntensity),
            }}
            transition={{
          ...
          <motion.div
            ...
            animate={{
              opacity: [0.6, 0.8, 0.6].map(v => v * glowIntensity * 0.8),
            }}
            transition={{
              duration: 5,
              repeat: Infinity,
