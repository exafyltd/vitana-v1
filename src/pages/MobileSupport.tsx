...
function MobileSupport() {
  ...
  if (showSuccess) {
    return (
      <MobileAppShell>
        <div className="px-4 pt-4 pb-6 h-[100dvh] overflow-y-auto bg-gradient-to-b from-background to-muted/30">
          <Card className="rounded-2xl border-border/50 shadow-sm mt-8">
  ...
  return (
    <MobileAppShell>
      <div className="px-4 pt-4 pb-6 h-[100dvh] overflow-y-auto bg-gradient-to-b from-background to-muted/30">
        ...
        {/* Hero: big mic button */}
        <Card className="rounded-2xl border-border/50 shadow-sm mt-3">
          <CardContent className="p-5 flex flex-col items-center text-center space-y-3">
            <p className="text-sm text-muted-foreground max-w-xs">{t("screens.mobileSupport.hint")}</p>
            ...
            <div className="flex items-center justify-center py-2">
              {!isRecording ? (
                ...
                <div className="flex flex-col items-center gap-3">
                  ...
                  <div className="text-xl font-mono font-bold text-destructive">{formatDuration(recordingDuration)}</div>
                </div>
              )}
            </div>
          ...
          <Card className="rounded-2xl border-border/50 shadow-sm mt-3">
            <CardContent className="p-4 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm font-semibold">{t("screens.mobileSupport.transcriptTitle")}</span>
              ...
              />
              <p className="text-xs text-muted-foreground">{t("screens.mobileSupport.speakNaturally")}</p>
            </CardContent>
          </Card>
        )}
        ...
        {transcript && !isRecording && (
          <Card className="rounded-2xl border-border/50 shadow-sm mt-3">
            <CardContent className="p-4 space-y-3">
              ...
              <p className="text-xs text-muted-foreground text-center">{t("screens.mobileSupport.responseTime")}</p>
            </CardContent>
          </Card>
        )}

        {/* Quick contact + help center */}
        <Card className="rounded-2xl border-border/50 shadow-sm mt-3">
          <CardContent className="p-4 space-y-3">
            <h3 className="text-sm font-semibold">{t("screens.mobileSupport.quickContactTitle")}</h3>
            <div className="grid grid-cols-3 gap-2">
              <button
                onClick={handleEmailClick}
                className="h-auto p-3 rounded-xl border border-border/50 hover:bg-muted/50 flex flex-col items-center gap-1.5 text-center"
              >
                <Mail className="w-5 h-5 text-primary" />
                <span className="font-medium text-xs">{t("screens.mobileSupport.quickContactEmail")}</span>
                <span className="text-[10px] text-muted-foreground break-all">
                  {t("screens.mobileSupport.quickContactEmailSub")}
                </span>
              </button>
              <div className="h-auto p-3 rounded-xl border border-border/50 flex flex-col items-center gap-1.5 text-center opacity-60">
                <MessageCircle className="w-5 h-5 text-primary" />
                <span className="font-medium text-xs">{t("screens.mobileSupport.quickContactChat")}</span>
              ...
              <div className="h-auto p-3 rounded-xl border border-border/50 flex flex-col items-center gap-1.5 text-center opacity-60">
                ...
                <span className="font-medium text-xs">{t("screens.mobileSupport.quickContactCall")}</span>
                <span className="text-[10px] text-muted-foreground">
        ...
        <button
          ...
          className="w-full mt-3 flex items-center gap-3 rounded-2xl px-4 py-3 bg-muted/40 border border-border/50 hover:bg-muted/60 transition-colors text-left"
        >
          <div className="w-9 h-9 rounded-xl flex items-center justify-center bg-primary/10">
            <BookOpen className="w-4 h-4 text-primary" />
          </div>
          <div className="flex-1 min-w-0">
            <span className="text-sm font-medium">{t("screens.mobileSupport.helpCenterTitle")}</span>
            <p className="text-xs text-muted-foreground">{t("screens.mobileSupport.helpCenterSub")}</p>
          </div>
        </button>
      </div>
    </MobileAppShell>
