/**
 * Partner Portal — connection detail: mapping review, sandbox tests, the
 * one-approval activation, lifecycle actions (VTID-03546, BLK-008).
 * All rules are server-side (VTID-03544); this screen presents them.
 */
...
export default function PartnerConnectionDetail() {
  ...
  const load = useCallback(async () => {
    try {
      const [d, p, a] = await Promise.all([
        adminFetch(`/api/v1/vcaop/portal/connections/${id}`),
        adminFetch(`/api/v1/vcaop/portal/connections/${id}/mapping-preview`),
        adminFetch(`/api/v1/vcaop/portal/connections/${id}/activation-summary`),
      ]);
      setDetail(d.data);
      setPreview(p.data);
  ...
  const act = async (key: string, path: string, init?: RequestInit) => {
    setBusy(key);
    try {
      await adminFetch(`/api/v1/vcaop/portal/connections/${id}${path}`, { method: 'POST', ...init });
      await load();
    } catch {
  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-4 p-4">
      <div className="flex flex-wrap items-center gap-3">
        <Button variant="ghost" size="icon" onClick={() => navigate('/partner/connections')} aria-label={t('screens.partnerportal.title')}>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <p className="text-xs text-muted-foreground">{t('screens.partnerportal.activationHint')}</p>
          <div className="flex flex-wrap gap-2">
            {activation?.can_activate && (
              <Button disabled={busy !== null} onClick={() => void act('activate', '/approve-activation')}>
                <ShieldCheck className="mr-2 h-4 w-4" />
                {t('screens.partnerportal.approveActivation')}
              </Button>
            )}
            {state === 'active' && (
