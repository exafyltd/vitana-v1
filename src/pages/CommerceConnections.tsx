/**
 * Commerce Portal — the merchant's OWN connections (VTID-03555). Thin client
 * over the gateway's owner-scoped /api/v1/vcaop/portal/my surface
 * (VTID-03553): the caller only ever sees businesses they created; the state
 * machine and mapping gate live server-side.
 */
      ...
      <Card>
        ...
        <CardContent>
          {rows === null ? (
            ...
            <p className="py-6 text-center text-sm text-muted-foreground">{t('screens.commerceportal.empty')}</p>
          ) : (
              ...
              {rows.map((row) => (
                <li key={row.id}>
                  <button
                    ...
                    onClick={() => navigate(`/commerce/connections/${row.id}`)}
                  >
