...
/**
 * Simple hash of a string to a number (for deterministic image selection).
 */
...
/**
 * Get a cover image URL for an article based on its tags.
 * Uses the article ID to deterministically pick from the category's image pool,
 * so different articles in the same category get different photos.
 */
